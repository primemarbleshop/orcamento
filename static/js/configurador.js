...
function drawEdgeLabelSmart(lines, x, y, isVert, nx, ny) {
    ...
    // REGRA CORRETA PARA TEXTOS VERTICAIS DO MESMO LADO:
    // mantém exatamente a mesma coluna X e afasta SOMENTE no eixo Y.
    ...
    const originalBox = getEdgeLabelBox(lines, x, y, isVert);
    const h = Math.max(1, originalBox.bottom - originalBox.top);
    const step = h + gap;
    const sameSideBoxes = () => edgeLabelBoxes.filter(b => b.sideKey === sideKey);

    let bestY = y;
    let bestBox = { ...originalBox, sideKey, isVert: true };

    const candidateYs = [y];
    for (let i = 1; i <= 20; i++) {
        // tenta primeiro para baixo, depois para cima, sempre mantendo o mesmo X
        candidateYs.push(y + i * step);
        candidateYs.push(y - i * step);
    }

    for (const cy of candidateYs) {
        const cbox = { ...getEdgeLabelBox(lines, x, cy, isVert), sideKey, isVert: true };
...
function getContourSegments(sections) {
    ...
    const sideForVertical = (x, outsideRight, y1, y2) => {
        ...
        // e ter exatamente a diferença de profundidade entre as bancadas vizinhas.
        // Ex.: seca 60 e molhada 40 => dente 20.
        ...
        if (leftNeighbors.length && rightNeighbors.length) {
            ...
            const dente = maxTop - minTop;
            const segLen = Math.abs(y2 - y1);
            // Só trata como dente do fundo quando o trecho vertical for exatamente
            // o desnível entre as duas bancadas que compartilham essa divisão.
            if (dente > 0.01 && Math.abs(segLen - dente) < 0.01 && midY >= minTop - 0.01 && midY <= maxTop + 0.01) {
                return 'fundo';
            }
        }
...
function drawEdgeMark(x1, y1, x2, y2, type, side, showLabel, bordaSide) {
    ...
    if (showLabel !== false) {
        ...
        ctx.fillStyle = color; ctx.font = 'bold 10px Inter,sans-serif';
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        const labelLines = edgeName(type, bordaSide);
        if (isVert) drawEdgeLabelSmart(labelLines, lx, ly, isVert, nx, ny);
        else drawEdgeLabel(labelLines, lx, ly, isVert);
    }
}
...
function drawLavViolao(W, H) {
    ...
    edgeSegs.forEach(seg => {
        ...
        const labelLines = edgeName(type, seg.side);
        if (isVert) drawEdgeLabelSmart(labelLines, mx+nx*soX, my+ny*soY, isVert, nx, ny);
        else drawEdgeLabel(labelLines, mx+nx*soX, my+ny*soY, isVert);
    });
